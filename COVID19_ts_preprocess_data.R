...
## -----------------------------------------------------------------------------
## Read and skim data
dat <- read.csv("./outputs/countyTable_timeSeries.csv")
skim(dat)
...
save(dat, file = "./outputs/covid19.RData")

## -----------------------------------------------------------------------------
## Read and skim new data
dat <- read.csv("./outputs/countyTable_timeSeries_conus.csv")
skim(dat)
...
## -----------------------------------------------------------------------------
## Merge census based state population values
state_test <- merge(state_test, state_popn, by.x = "state.x", by.y = "Abbr")
#state_test <- state_test %>%
#  select(-State, -sFIPS)
...
## -----------------------------------------------------------------------------
## Merge state test numbers back to main data frame
dat <- merge(dat, state_test, by = c("state.x", "date"))
...
newdat <- dat
save(newdat, file = "./outputs/covid19new.RData")
